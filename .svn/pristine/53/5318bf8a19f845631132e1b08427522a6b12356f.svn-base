...
public class AdvancedUtil {
    ...
    public static WeixinOauth2Token getOauth2AccessToken(String appId, String appSecret, String code) {
        ...
        // 拼接请求地址
        String requestUrl = "";
        requestUrl = StaticValue.REQUESTURL.replace("APPID", appId).replace("SECRET", appSecret).replace("CODE", code);
        // 获取网页授权凭证
    ...
    public static SNSUserInfo getSNSUserInfo(String accessToken, String openId) {
        ...
        // 拼接请求地址
        String requestUrl = "";
        requestUrl = StaticValue.GETUSERINFO.replace("ACCESS_TOKEN", accessToken).replace("OPENID", openId);
        // 通过网页授权获取用户信息
        JSONObject jsonObject = CommonUtil.httpsRequest(requestUrl, "GET", null);
        ...
        if (null != jsonObject) {
            try {
                ...
                // 用户特权信息
                snsUserInfo.setPrivilegeList(JSONObject.parseArray(JSONObject.toJSONString(jsonObject.getJSONArray("privilege")),String.class));
            } catch (Exception e) {
                snsUserInfo = null;
                int errorCode = jsonObject.getIntValue("errcode");
                String errorMsg = jsonObject.getString("errmsg");
